...
Bu ADR boyunca payload olarak anılan değer, bir tünelin eksiksiz modeli olan `TunnelConfig` yapısıdır:
...
Tünel sırları, **Sistem Uzantısı** sahipliğindeki bir **System Keychain** kasasında saklanır; bu kasa ürün genelinde **TunnelVault** adını taşır. Sistemin Network Extension deposu, sır taşımayan bir izdüşüme indirgenir ve iki depo arasındaki hizalama tek yönlü bir doğruluk ilişkisiyle tanımlanır: **kasa doğruluk kaynağıdır**. Uygulama bu depoya yalnız bir oturum el sıkışması üzerinden bakar. Bu el sıkışmasının iki tarafı vardır: uygulama tarafında oturumu yöneten `TunnelVaultSession`, çağrıyı `TunnelVaultClient` üzerinden gönderir; uzantı tarafında ise **PhantomTunnel** içindeki `TunnelVaultDaemon` cevap verir. Bağlantının kurulması launchd üzerinden uzantıyı uyandırır; el sıkışması tamamlanmadan tünel arayüzü açılmaz.
...
2. **Yazan taraf uzantıdır, uygulama değil.** System Keychain root sahipliğindedir ve uygulama sandbox içinde bir kullanıcı sürecidir. Bu yüzden uygulama kasaya hiç dokunmaz; `TunnelVaultClient` üzerinden uzantıya sorar. `TunnelVaultDaemon`, `ProxyConfigDaemon` ile aynı iskelettedir: `Info.plist` içindeki `NEMachServiceName` kaydı sayesinde launchd uzantıyı talep üzerine uyandırır, `shouldAcceptNewConnection` gelen bağlantıyı takımımızın imzasına bağlar.
...
5. **Provider kendi okumasını süreç içinde yapar.** `startTunnel`, kimliği `providerConfiguration` üzerinden alır ve payload gövdesini `SystemKeychainVault.fetchForProvider(id:)` ile doğrudan okur. XPC yoktur, diske düşen açık kopya yoktur, sır yalnızca bellekte çözülür.
