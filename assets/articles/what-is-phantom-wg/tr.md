...
![Phantom-WG Kurulum](assets/installation-dark.gif)
...
```mermaid
flowchart TD
    start(["☀️ Güne Başla"]) --> connect["Temel İstemci Bağlantı Döngüsü<br/>wstunnel - WireGuard Client"]
    connect --> choice{"Hangi çalışma ortamı?"}
    ...
    choice -->|"🌐 Genel İnternet"| direct["phantom-api multihop<br/>disable_multihop"]

    us --> work_us["NovaBridge<br/>CI/CD, Code Review, Staging"]
    ca --> work_ca["MapleSoft<br/>CI/CD, Code Review, Staging"]
    direct --> browse["Genel İnternet<br/>Türkiye Çıkışı"]

    work_us --> switch{"Ortam Değiştir?"}
    work_ca --> switch
    browse --> switch

    switch -->|"Evet"| choice
    switch -->|"Hayır"| continue["Çalışmaya Devam Et"]
