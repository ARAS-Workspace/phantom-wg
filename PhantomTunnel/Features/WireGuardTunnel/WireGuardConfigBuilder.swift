...
enum WireGuardConfigBuilder {
    ...
    /// from the Keychain blob — so this builder re-checks what
    ...
    private static func buildPeer(from config: WireguardConfig, wstunnel: WstunnelConfig?) throws -> PeerConfiguration {
        ...
        // AllowedIPs, but a hand-edited or malformed payload can still
        // arrive here with none; refuse to start rather than run a
        // tunnel that routes nothing.
        ...
        // without one can only be a hand-damaged blob — refuse to start.
