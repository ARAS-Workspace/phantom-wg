...
final class PhantomTestEngine {
    ...
    func run(_ workflows: [TestWorkflow], _ ctx: TestContext) async {
        // Belt to the view's disabled Run button: a second entry (e.g.
        // sheet reopened mid-run) must never interleave two runs over
        // the same live services.
        ...
        emit("", .info)
        emit(Task.isCancelled ? "— stopped —" : "— done —", .result)
    }
}
#endif
